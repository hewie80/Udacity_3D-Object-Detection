...
def show_pcl(pcl,cnt_frame = 0):
    ...
    o3dView = o3d.visualization.VisualizerWithKeyCallback()
    o3dView.create_window(window_name='Point Cloud View', width=1920, height=1080, left=50, top=50, visible=True)
    ...
    if cnt_frame == 0:
        o3dView.add_geometry(pcd)
    else:
        o3dView.update_geometry(pcd)
    
    # step 5 : visualize point cloud and keep window open until right-arrow is pressed (key-code 262)
    o3d.visualization.draw_geometries([pcd])
    o3dView.register_key_callback(262,right_click)
...
def show_range_image(frame, lidar_name):
    ...
    # step 6: Crop range image to +/- 90 deg. left and right of the forward-facing x-axis
...
#     show_pcl(lidar_pcl_cpy)
    ...
    index, unique,counts = np.unique(lidar_pcl_cpy[:, 0:2].astype(np.int), axis=0, return_index=True,return_counts=True)
