...
while True:
    try:
        ...
        if 'show_range_image' in exec_list:
            img_range = pcl.show_range_image(frame, lidar_na)
            img_range = img_range.astype(np.ut8)
            cv2.imshow('range_image', imgange)
            cv2.waitKey(vis_pau_time)

        if 'show_pcl' in exec_list:
            pcl.show_pcl(lidar_p,cnt_frame)
            cv2.waitKey(v_pause_time)

        if 'show_b' in exec_list:
            tools.show_bev(lidar_v, configs_det)  
            cv2.waitKey(vis_pse_time)          

        if 'show_labels__image' in exec_list:
            img_labels = tools.project_labels_into_camera(camera_calibration, image, frame.laser_labelsvalid_label_flags, 0.5)
            cv2.imshow('img_labels', img_labels)
            cv2.waitKey(vis_pause_time)

        if 'show_objects_a_labels_in_bev' in exec_list:
            tools.show_objects_labels_in_bev(detections, frame.laselabels, lidar_bev, configs_det)
            cv2.waitKey(vis_pause_time)         

        if 'show_objects_ibev_labels_in_camera' in exec_list:
            tools.show_objects_in_bev_labels_in_camera(detections, lidar_bev, image, frame.laser_labels, valid_label_ags, camera_calibration, configs_det)
            cv2.waitKey(vis_pause_time)               
